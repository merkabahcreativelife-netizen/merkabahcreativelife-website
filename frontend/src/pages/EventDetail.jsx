...
export default function EventDetail() {
  ...
  if (!e) return <div className="pt-32 pb-24 text-center text-ink-mute">Event not found.</div>;
  return (
    <>
      <section className="relative min-h-[60vh] flex items-end pb-16 overflow-hidden">
        {e.image && <img src={e.image} alt="" className="absolute inset-0 w-full h-full object-cover opacity-70" />}
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8 w-full">
          ...
          <h1 className="font-display font-light text-4xl sm:text-6xl lg:text-7xl leading-[0.95] tracking-tighter text-ink mt-4 max-w-4xl">{e.name}</h1>
        </div>
      </section>
      <section className="py-20 max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-3 gap-10">
        ...
        <div className="border border-cream-200 p-8 h-fit space-y-5">
          ...
          {e.upcoming && (
            <Link to="/contact" data-testid="event-register" className="block text-center bg-terracotta-500 hover:bg-terracotta-600 text-white py-4 text-xs uppercase tracking-[0.25em] transition-colors mt-4">Register Interest</Link>
          )}
        </div>
      </section>
    </>
