  ...
  return (
    <>
      <section className="relative min-h-[70vh] flex items-end pb-20 overflow-hidden">
        <img src="https://images.unsplash.com/photo-1470229722913-7c0e2dbbafd3?crop=entropy&cs=srgb&fm=jpg&q=85" alt="" className="absolute inset-0 w-full h-full object-cover opacity-70" />
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <h1 className="font-display font-light text-5xl sm:text-7xl lg:text-8xl leading-[0.95] tracking-tighter text-ink mt-6">Live. Loud. Together.</h1>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {upcoming.map(e => (
              <Link to={`/live/${e.id}`} key={e.id} data-testid={`event-${e.id}`} className="block border border-cream-200 p-8 hover:border-terracotta-500 transition-colors">
                ...
                <p className="text-ink-mute text-sm">{e.description}</p>
              </Link>
            ))}
          </div>
        )}
