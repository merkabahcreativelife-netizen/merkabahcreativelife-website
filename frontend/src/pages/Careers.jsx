  ...
  return (
    <>
      <section className="pt-24 pb-16 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <h1 className="font-display font-light text-5xl sm:text-7xl lg:text-8xl leading-[0.95] tracking-tighter text-ink mt-4 max-w-5xl">Create your next chapter with Merkabah.</h1>
        <p className="mt-8 max-w-2xl text-lg text-ink-soft">We're building a creative ecosystem — and we're looking for curious people to build it with us.</p>
      </section>
      ...
      <section className="py-16 border-t border-silver-200 max-w-7xl mx-auto px-6 lg:px-8">
            ...
            {jobs.map(j => (
              <li key={j.id}>
                <Link to={`/careers/job/${j.id}`} data-testid={`job-${j.id}`} className="py-6 flex items-center justify-between group">
                  <div><div className="font-display text-xl text-ink group-hover:text-terracotta-400 transition-colors">{j.title}</div><div className="text-ink-mute text-sm mt-1">{j.department} · {j.employment_type} · {j.location}</div></div>
        ...
        <Link to="/careers/internships" data-testid="internships-cta" className="mt-14 inline-block bg-terracotta-500 hover:bg-terracotta-600 text-white px-8 py-4 text-xs uppercase tracking-[0.25em]">Apply for an Internship</Link>
      </section>
    </>
  );
}
