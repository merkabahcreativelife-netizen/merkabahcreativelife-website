...
import { FOUNDER_IMG, HEALER_CARD_IMG } from "@/data/worlds";
  ...
  return (
    <>
      <section className="relative min-h-[70vh] flex items-end pb-20 overflow-hidden">
        <img src="https://images.unsplash.com/photo-1506126613408-eca07ce68773?crop=entropy&cs=srgb&fm=jpg&q=85" alt="" className="absolute inset-0 w-full h-full object-cover opacity-35" />
        ...
        <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
          ...
          <h1 className="font-display font-light text-5xl sm:text-7xl lg:text-8xl leading-[0.95] tracking-tighter text-cream-50 mt-6 max-w-4xl">A space to pause. Reset. Reconnect.</h1>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <h2 className="font-display text-4xl lg:text-6xl text-cream-50 font-light tracking-tight leading-[1.05] mb-14 max-w-3xl">Ancient practices, held with a modern heart.</h2>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {MODALITIES.map(([t, d]) => (
            <div key={t} className="border border-stone-800 p-8 hover:border-terracotta-500 transition-colors" data-testid={`wellness-modality-${t.toLowerCase().replace(/ /g, "-")}`}>
              <div className="font-display text-2xl text-cream-50 mb-4">{t}</div>
              <p className="text-stone-400 leading-relaxed text-sm">{d}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-24 border-y border-stone-900 bg-stone-900/30">
        <div className="max-w-7xl mx-auto px-6 lg:px-8 grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className="max-w-md mx-auto lg:mx-0 w-full space-y-8">
            <div className="rounded-full overflow-hidden border border-terracotta-500/30 p-3 bg-stone-950">
              <img src={FOUNDER_IMG} alt="Kingsley Victor" className="w-full aspect-square object-cover rounded-full" />
            </div>
            <img src={HEALER_CARD_IMG} alt="Merkabah Magic & Miracles" className="w-full max-w-xs mx-auto border border-stone-800" />
          </div>
          <div>
            <div className="overline mb-4">Your Guide</div>
            <h2 className="font-display text-4xl lg:text-5xl text-cream-50 font-light tracking-tight leading-tight">Kingsley Victor</h2>
            ...
            <div className="mt-8 space-y-5 text-stone-300 leading-relaxed">
              ...
              <p className="font-display text-xl italic text-cream-100">"He always nurtured a natural wish to uplift others through his spiritual development."</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 max-w-7xl mx-auto px-6 lg:px-8">
        ...
        <h2 className="font-display text-4xl lg:text-6xl text-cream-50 font-light tracking-tight leading-[1.05] mb-14 max-w-3xl">Choose the door that calls you.</h2>
        ...
        <div className="mt-16 border border-terracotta-500/30 p-10 lg:p-14 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div>
            <div className="font-display text-3xl text-cream-50 font-light">Begin with a conversation.</div>
            <p className="text-stone-400 mt-3 max-w-lg">Tell us what you are carrying, and we will suggest the experience that fits.</p>
          </div>
          <Link to="/contact" data-testid="wellness-cta" className="shrink-0 bg-terracotta-500 hover:bg-terracotta-600 text-cream-50 px-8 py-4 text-xs uppercase tracking-[0.25em] text-center transition-colors">Enquire / Book a Session</Link>
        </div>
      </section>
    </>
  );
